...
func runProcess(timeoutVar *time.Duration, args []string) (err error) {
	...
	client := pkg.TelenetClientInit(net.JoinHostPort(host, port), *timeoutVar, os.Stdin, os.Stdout)
...
func processSend(client *pkg.TelnetClientInt, errorCh chan error) {
	for {
		err := (*client).Send()
		if err != nil {
			errorCh <- err
...
func processReceive(client *pkg.TelnetClientInt, errorCh chan error) {
	for {
		err := (*client).Receive()
		if err != nil {
			errorCh <- err
